...
def create_app():
    ...
    def _det_query():
        ...
        Regular users see only their own detections (uploads + their own ESP32 token).
        """
    ...
    def esp32_data():
        ...
        token_str = payload.get("token") or request.headers.get("X-Device-Token")
        if not token_str:
            return jsonify({"error": "Missing device token. Include 'token' in payload or X-Device-Token header."}), 401

        device_token = DeviceToken.query.filter_by(token=token_str).first()
        if not device_token:
            return jsonify({"error": "Invalid device token."}), 403

        owner = device_token.user
        if not owner or not owner.is_active:
            return jsonify({"error": "Token owner account is inactive."}), 403
        ...
        det = Detection(predicted_class=predicted_class, class_name=class_name,
            ...
            device_id=device_id, location=location, user_id=owner.id)
        db.session.add(det)
        db.session.commit()
    ...
    @login_required
    def device_token():
        token = DeviceToken.get_or_create(current_user.id)
        return render_template("device_token.html", token=token)

    @app.route("/settings/token/regenerate", methods=["POST"])
    @login_required
    def regenerate_token():
        token = DeviceToken.get_or_create(current_user.id)
        token.regenerate()
        flash("Device token regenerated successfully. Update your ESP32 with the new token.", "success")
        return redirect(url_for("device_token"))
...
def _migrate_db():
    ...
    # Add user_id to detections if missing
    ...
    # Auto-generate device tokens for all existing users (except system)
    users = User.query.filter(User.role != "system").all()
    for user in users:
        if not DeviceToken.query.filter_by(user_id=user.id).first():
            token = DeviceToken(user_id=user.id)
            db.session.add(token)
    db.session.commit()
    print("[CSI] Device tokens ensured for all users")
...
def _seed_users():
    ...
    # Ensure admin has a device token
    admin = User.query.filter_by(username="admin").first()
    if admin and not DeviceToken.query.filter_by(user_id=admin.id).first():
        token = DeviceToken(user_id=admin.id)
        db.session.add(token)
        db.session.commit()
        print("[CSI] Device token created for admin")
