...
def _build_model():
    ...
    dropout = 0.3
    inp = tf.keras.Input(shape=(TIME_STEPS, SUBCARRIERS, 1), name="csi_input")
    ...
    lstm1 = layers.Bidirectional(layers.LSTM(256, return_sequences=True, dropout=0.2, recurrent_dropout=0.1), name="bilstm1")(x)
    lstm2 = layers.Bidirectional(layers.LSTM(128, return_sequences=True, dropout=0.2, recurrent_dropout=0.1), name="bilstm2")(lstm1)
    res = layers.Dense(256, name="res_proj")(lstm1)
    x   = layers.Add(name="res_add")([lstm2, res])
    x   = layers.LayerNormalization(dtype="float32", name="layer_norm")(x)
    ...
    model = tf.keras.Model(inputs=inp, outputs=out, name="CSI_CNN_BiLSTM_FIXED")
...
def _load_model(path):
    ...
    try:
        ...
        print(f"[CSI] Keras model loaded via rebuild + load_weights from {path}")
        return model
    except Exception as e1:
        print(f"[CSI] rebuild+load_weights failed: {e1}")
    try:
        model = tf.keras.models.load_model(path, compile=False)
        model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4),
                      loss="categorical_crossentropy", metrics=["accuracy"])
        print(f"[CSI] Keras model loaded via load_model(compile=False) from {path}")
        return model
    except Exception as e2:
        print(f"[CSI] load_model failed: {e2}")
    print("[CSI] All load attempts failed. Model NOT loaded.")
    return None
